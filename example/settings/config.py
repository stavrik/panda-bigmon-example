...
#DATABASE_ROUTERS = ['example.dbrouter.ExampleDBRouter']
...
#    join(dirname(example.__file__), 'static'),
)
...
TEMPLATE_DIRS = (
    ...
    # Always use forward slashes, even on Windows.
    # Don't forget to use absolute paths, not relative paths.
    join(dirname(example.__file__), 'templates'),
    join(dirname(core.common.__file__), 'templates'),
)

STATIC_ROOT = join(dirname(example.__file__), 'static')
#STATIC_ROOT = None
MEDIA_ROOT = join(dirname(example.__file__), 'media')
